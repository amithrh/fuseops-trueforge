...
`scripts/register-agent.ts` saves the `fuseops-commander` manifest with the control-plane connector, approval required for `rollback_deployment`, sandbox enabled, dynamic subagents enabled, Generative UI enabled, and an iteration cap. The focused instructions require evidence before action, reserve remediation for the root commander, bound subagents to evidence review, and require a sandbox correlation calculation. The locally verified model profile uses `reasoning_effort: none` and a 3,000-token response cap.
...
4. TrueForge delegates hypotheses and uses its isolated local sandbox for a correlation script.
...
- **Local model latency/tool compliance:** CPU-only Docker inference failed to emit a tool call within the demo window. Native Metal with `reasoning_effort: none` completed the verified path in 2m12s; retain replay for portability and require a fresh live smoke when hardware or provider changes.
...
The submission demo requires a clean reset, both workspace apps and TrueForge, a responsive tool-capable model, the recommended prompt, visible delegation/sandbox work, the real Allow/Deny gate, an approved mutation, and the recovered health strip plus audit record. This flow passed locally in session `01m19qc2w5b87hpa6z9dj0ds6y`; replay may supplement the story but cannot replace a fresh recorded live run.
